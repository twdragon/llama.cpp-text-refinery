...
OUTPUT_FILE_SUMMARY = TRANSCRIPT_FILE.parent.joinpath(str(TRANSCRIPT_FILE.stem + '.summary.md'))
OUTPUT_FILE_OUTLINE = TRANSCRIPT_FILE.parent.joinpath(str(TRANSCRIPT_FILE.stem + '.outline.md'))
...
if batched:
    for i, batch in enumerate(batches):
        log.info('Rendering batch {}, max {}'.format(i, len(batches) - 1))
        # Generates outline
        response_text = str()
        payload_prompt = outline_prompt + ' '.join(batch) + '\n\nResponse: \n\n'
        ...
        with requests.post(LLAMASERVER_GENERATION_ENDPOINT, json=payload, stream=True) as response:
            if response.status_code == 200:
                ...
                print()
                response_text += '\n\n'
                raw_outlines.append(response_text)
            else:
                log.error('HTTP error: status code {}'.format(response.status_code))
        # Generates summary
        summary_text = str()
        payload_prompt = summary_prompt + ' '.join(batch) + '\n\nResponse: \n\n'
        payload = {'prompt': payload_prompt, 
            'stream': True, 
            'temperature': 0.5,
            'n_predict': 1750,
        ...
        with requests.post(LLAMASERVER_GENERATION_ENDPOINT, json=payload, stream=True) as response:
            if response.status_code == 200:
                ...
                print()
                summary_text += '\n\n'
                raw_summaries.append(summary_text)
            else:
                log.error('HTTP error: status code {}'.format(response.status_code))
        log.info('Batch {} processed'.format(i))

log.info('Writing summary to {}'.format(str(OUTPUT_FILE_SUMMARY)))
summary_file = OUTPUT_FILE_SUMMARY.open('wt', encoding='utf-8')
for summary in raw_summaries:
    summary_file.write(summary)
summary_file.close()
log.info('Writing outline to {}'.format(str(OUTPUT_FILE_OUTLINE)))
outline_file = OUTPUT_FILE_OUTLINE.open('wt', encoding='utf-8')
